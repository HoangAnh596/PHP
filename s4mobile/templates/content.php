<div class="col-md-9 bor">
    <section id="slide" class="text-center">
        <button id="nutBack" onclick="back()" type="button" class="btn btn-outline-info">&#x27F5;</button>
        <a href="" id="link_img"><img src="" alt="Lỗi ảnh" id="anh" width="750" height="350" style="margin-top:10px;"></a>
        <button id="nutNext" onclick="next()" type="button" class="btn btn-outline-info">&#x27F6;</button>
    </section>
    <?php
    ...
    if($total_records==0){
        ...
        $stmt = $objConn->prepare("SELECT * FROM products LIMIT $start,$limit");
        $stmt->execute();
        $stmt->setFetchMode(PDO::FETCH_ASSOC);
        $products = $stmt->fetchAll();
    }
    ?>
    <section class="box-main1" id="allPro">
        <h3 class="title-main" style="text-align: center;"><a href="?page=search&allProduct"> Tất cả loại máy
            </a> </h3>
        <div class="showitem">
            <?php
  ...
  <?php
    if($current_page>1 && $total_page>1){
      echo '<a href="index.php?page&page_control='.($current_page-1).'"><button type="button" class="btn btn-outline-info" style="margin-right:5px;">Prev</button></a>';
    }
    for($i=1;$i<=$total_page;$i++){
      if($i==$current_page){
          echo "<button type=\"button\" class=\"btn btn-outline-dark\" style=\"margin-right:5px;color:black;\">$i</button>";
      }
      else{
          echo '<a href="index.php?page&page_control='.$i.'"><button type="button" class="btn btn-outline-info" style="margin-right:5px;">'.$i.'</button></a>';
      }
    }
    if ($current_page < $total_page && $total_page > 1){
      echo '<a href="index.php?page&page_control='.($current_page+1).'"><button type="button" class="btn btn-outline-info" style="margin-right:5px;">Next</button></a>';
    }
  ?>
</div>